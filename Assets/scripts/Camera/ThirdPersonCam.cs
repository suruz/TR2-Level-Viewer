	...
	void Update () 
	{
		...
		else
		{
			if (anim.m_SwimState == SwimmingState.InDeepWater)
			{
				...
				} else { Vector3 pos = target_hip.position - Vector3.up * verticalOffset; m_Transform.forward = target.forward + Vector3.up * lookup; m_Transform.position = new Vector3(pos.x, pos.y + 128 * Settings.SceneScaling, pos.z) - m_Transform.forward * horizontalOffset; }
			}
			else
			{
				...
				if (!Settings.bUseFirstPersonCamera){
					...
					Vector3 pos = target_hip.position - forward * distance;
					m_Transform.position = Vector3.Lerp(m_Transform.position, new Vector3(pos.x, pos.y + 128 * Settings.SceneScaling, pos.z), Time.deltaTime * 2.0f);
				} else { Vector3 pos = target_hip.position - Vector3.up * verticalOffset; m_Transform.forward = target.forward + Vector3.up * lookup; m_Transform.position = new Vector3(pos.x, pos.y + 400 * Settings.SceneScaling, pos.z) - m_Transform.forward * horizontalOffset; }
			}
